...
import sys
import pickle as pc
h='/home/sdhawan/Downloads/CSP_Photometry_DR2/CSP_Photometry_DR2/'
...
class conv:					
	"""
	#class reads in the dat file and stores it as a python dictionary. the second function							
	#converts dicts to a 	panel after converting them to dataframes
...
class spl_fit:
	def rd_cfa(self, sn, band):
		"""
		Read the CfA light curves (slightly different file format)
	...
	def bdmax(self, sn, band, sur):				
		"""
		Maximum in a given band for a given SN from a survey (for file format)
			
		Returns 99.0 is maximum not observed (try SNPy template fit then, its pretty cool)
		"""
		...
		spl=interp1d(lc1['MJD'], lc1[band], kind='cubic')
		...
		if min(lc1['MJD']-tm)<0:
			...
			return 99.0, 99.0
	def lt_dec(self, sn, band, sur, ran=[40.0, 90.0]):
		"""
		
		Measurement of the decline rate for a set of objects with observations between +40 and +90 days
		"""
		lc1=self.rd_cfa(sn, band)
		#tm=self.bdmax(sn, band, sur)[1]
		ph=lc1['ph']
		ph1=ph[(ph>=ran[0]) & (ph<=ran[1])]
		mag=lc1[band][(ph>=ran[0]) & (ph<=ran[1])]
...
class rdbol:
	...
	Read the evaluated bolometric light curve
...
class t2corr:
	...
	Print correlation between Mni (from Richard's mcmc) and t2(X)
...
class interp:
	...
	pt='/home/sdhawan/bol_ni_ej/comp_ddc/'
	...
	def bolpeak(self, fil):
		...
		Determine bolometric peak, same as bolpeak in bol in the package (innovatively) named 'pack'
		"""
		pn=np.loadtxt(rdbol().pt+fil)#'lcbol_distrib/upeak/'+sn+'_lc_bolu.dat')
		spl=interp1d(pn[:,0], pn[:,1], kind='cubic')
		l1=np.linspace(min(pn[:,0]), max(pn[:,0]), 100)
		gpl=spl(l1)
		return l1[gpl==max(gpl)][0], max(gpl)
