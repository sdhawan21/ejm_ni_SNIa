...
def lbol_red(tval):
	...
	print "The number of objects in the fit is: \t", len(t2), "\n \n"
	...
	ar=[(np.random.normal(val[0], err[0])*np.random.normal(tval[0], tval[1])+np.random.normal(val[1], err[1]))/np.random.normal(2., .3) for i in range(3000)]
...
def main():
	...
	ev=et                              
	ar=lbol_red([v, ev])
	print np.mean(ar), np.std(ar), fac, skew(ar)#, min(ar), max(ar)#np.mean(ar)/2, (np.mean(ar)/2)*((np.std(ar)/np.mean(ar))+(0.3/2.0))
